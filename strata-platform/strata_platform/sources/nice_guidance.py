"""Per-TA NICE guidance fetcher + pure parser.

The spreadsheet gives the universe at fiscal-year granularity; the per-TA guidance page
gives the EXACT decision date and the committee rationale (the gold-bearing text). One
fetch serves both.
...
from strata_platform.sources.endpoints import BROWSER_UA, NICE_BASE
from strata_platform.substrate.contracts import DocType, SourceRecord
from strata_platform.substrate.provenance import SnapshotStore, snapshot
...
def guidance_url(ta_id: str) -> str:
    """The recommendations CHAPTER, not the overview page. The overview is a shell; the
    date AND the rationale both live on /guidance/{ta}/chapter/1-recommendations."""
...
_RATIONALE_MARKERS = ("1 Recommendation", "Recommendation")
_WHY_MARKERS = (
...
def _largest_section(text: str, marker: str, max_chars: int = 8000) -> str:
    ...
    next top-level numbered heading. NICE chapter pages open with a table-of-contents that
    repeats every heading, so the FIRST occurrence is a nav link; taking the longest skips
    the nav and lands on the real content."""
    best = ""
    start = 0
    while True:
        ...
        nxt = _SECTION.search(tail, len(marker) + 1)
...
def _extract_rationale(text: str) -> str:
    """The Recommendations section (the verdict + committee discussion). Falls back to the
    explicit 'Why the committee…' block. Blocks under 60 chars are nav fragments."""
...
def _default_cache_dir() -> Path:
    return Path(get_settings().local_blob_dir).parent / "nice_guidance"
...
class NICEGuidanceClient:
    ...
    def __init__(
        ...
        *,
        ...
        self._snap = SnapshotStore()

    def _cache_path(self, ta: str) -> Path:
        return self._cache_dir / f"{ta}.json"
    ...
    def _load_cache(self, ta: str) -> GuidanceResult | None:
        ...
        # An 'ok' pointer is only valid if its content snapshot still exists.
        if result.status == "ok" and result.source_record is not None:
            if not self._snap.exists(result.source_record.content_sha256):
                return None
        return result
    ...
    def fetch(self, ta_id: str) -> GuidanceResult:
        ...
        result = GuidanceResult(ta_id=ta, status="ok", parsed=parsed, source_record=rec)
