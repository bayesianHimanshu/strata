"""Task 2.2: pure guidance parser (fixtures) + resilient fetcher (no network)."""
...
from sources.nice_guidance import (
    NICEGuidanceClient,
    guidance_url,
    is_withdrawn,
    parse_guidance,
...
def test_parse_guidance_time_element() -> None:
    ...
    # bounded at the next numbered section
...
def test_parse_guidance_published_text_and_plain_heading() -> None:
    ...
    assert pg.published_date == date(2026, 3, 3)  # from "Published: 3 March 2026"
...
def test_parse_guidance_real_chapter_skips_nav_toc() -> None:
    # Real chapter pages repeat every heading in a nav ToC; the parser must land on
    # the substantive section, not the nav fragment, and handle the singular heading.
    pg = parse_guidance(_html("ta_chapter.html"), "TA1064")
    assert pg.published_date == date(2025, 5, 22)
    low = pg.rationale_raw.lower()
    assert "1.1 dostarlimab" in low  # real content, not the nav link
    assert "immature" in low and "indirect comparison" in low and "icer" in low
    assert "Marketing authorisation" not in pg.rationale_raw  # bounded at section 2
...
def _fetcher(tmp_path: Path, responses: dict[str, tuple[int, bytes, str]]):
    ...
    client = NICEGuidanceClient(
        http_get=fake_get, snapshot_root=tmp_path, sleeper=lambda _s: None
    )
    return client, calls
...
def test_fetch_404_is_graceful_and_cached(tmp_path: Path) -> None:
    ...
    assert len(calls) == 1  # withdrawn TA not refetched
