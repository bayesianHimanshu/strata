"""Pure parsers added for the retrieval corpus: PubMed efetch + openFDA labels."""
from __future__ import annotations

from datetime import date

from sources.openfda import parse_label_docs
from sources.pubmed import parse_efetch
...
def test_parse_efetch_extracts_title_abstract_date() -> None:
    ...
    assert a.pub_date == date(2024, 3, 15)  # Year/Month/Day, abbreviated month
...
def test_parse_label_docs_text_and_date() -> None:
    ...
    assert d.effective_date == date(2024, 3, 15)  # compact YYYYMMDD parsed
