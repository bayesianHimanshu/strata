...
def build_inputs(
    decisions_path: str, gold_path: str
...
def main() -> int:
    ...
    cfg = RunConfig(
        model=args.model, model_cutoff=GPT55_CUTOFF, max_tokens=args.max_tokens
    )
    reasoner = OpenAIReasoner(cfg, reasoning_effort=args.reasoning_effort)

    report = run_probe(
        decisions, gold, reasoner, cfg, taxonomy, rubric_hash=rubric_hash()
    )
    out = report.to_dict()
    ...
    out["strict_post_slice"] = {
        ...
        **strict_slice_recall(
            decisions, gold, report.closed_book_predictions, taxonomy, STRICT_CUTOFF
        ),
    }
    ...
    snapshot(
        ...
        source="probe",
        source_id="closed_book_gpt55",
        url="local://probe",
    )

    md = out["metadata"]
    print(
        f"model={md['model']} cutoff={md['model_cutoff']} "
        f"n_pre={md['n_pre']} n_post={md['n_post']}"
    )
    print("closed-book POST micro recall:", out["closed_book"]["post"]["micro_recall"])
    print(
        f"strict POST (>{STRICT_CUTOFF}) micro recall:",
