...
def chunk_doctype(c):
    return _f(c, "doc_type", "type", default="?")


def chunk_appraisal(c):
    return _f(c, "appraisal_id", "ta_id", "ta")


def chunk_drug(c):
    return _f(c, "drug", "technology", "source_drug")


def chunk_text(c):
    return _f(c, "text", "content", "chunk_text", "chunk", default="")
...
def composition(chunks: list[dict]) -> dict:
    ...
    return {
        ...
        "date_range": (
            [min(dates).isoformat(), max(dates).isoformat()] if dates else None
        ),
    }


def eligible_pool(
    decision: dict, chunks: list[dict], buffer_days: int, *, match_drug: bool
) -> list[dict]:
...
def main() -> int:
    ...
    ap.add_argument(
        "--snippets", type=int, default=2, help="evidence snippets per focus decision"
    )
    args = ap.parse_args()
    ...
    if comp["literature_missing"]:
        print(
            ">>> WARNING: zero literature chunks - the PubMed arm did not land. "
            "Open-book is grounding on trials/labels only."
        )
    if not has_drug:
        print(
            ">>> NOTE: chunks carry no drug field - eligible pool is date+dossier "
    ...
    for r in rows:
        ...
        print(f"  gold={r['gold']}")
        print(
            f"  open missed (closed got these, open lost): "
    ...
    for r in focus:
        ...
        ev = [
            c
            for c in r["_eligible_objs"]
            if chunk_doctype(c) in ("literature", "pubmed", "label")
        ][:n_snip]
        if not ev:
    ...
    print(
        "  DISCIPLINE: public data lacks the committee's reasoning; "
        "that's the finding."
    )
    return 0
