...
def _micro(ids, gold, open_pred, closed_pred, taxonomy):
    ...
    block = {
        ...
        "recall_open": orr,
        "recall_closed": crr,
        "delta_recall": _delta(orr, crr),
        "precision_open": op,
        "precision_closed": cp,
        "delta_precision": _delta(op, cp),
    }
...
def _headline(micro: dict) -> str:
    ...
    if d_prec is not None and d_prec > 0 and (d_rec is None or d_rec >= -0.05):
        return (
            "retrieval RAISES precision without losing recall - the retrieval "
            "system earns its place over parametric priors"
        )
    if d_prec is not None and d_prec <= 0:
        return (
            "retrieval does NOT improve precision over parametric priors - "
...
def run_open_book(
    ...
    *,
    ...
    open_pred = {
        d.decision_id: synth.predict_open_book(d, buffer_days=buffer_days) for d in post
    }
    ...
    for c in taxonomy:
        ...
        by_cat[c] = {
            ...
            "recall_open": o["recall"],
            "recall_closed": cl["recall"],
            "precision_open": o["precision"],
            "precision_closed": cl["precision"],
...
def main() -> int:
    ...
    def _show(label: str, m: dict) -> None:
        ...
        print(
            f"  recall    open={m['recall_open']} closed={m['recall_closed']} "
            f"Δ={m['delta_recall']}"
        )
        print(
            f"  precision open={m['precision_open']} closed={m['precision_closed']} "
